...
from convert_h265_to_mp4_jpg import *
import json 
import pytz
SHANGAI = pytz.timezone('Asia/Shanghai')

f_o = open("key1.json")
#-----------------------------------------
key_o = json.load(f_o)
ip=key_o['url']# IP address of your cam
port=key_o['port']
user = key_o['user']
pwd = key_o['pwd']
rawpath = key_o['path']
adr="rtsp://"+ip+":"+str(10998)+"/onvif1" # username, passwd, etc.
clientports=[60784,60785] # the client ports 
...
def retrieve_jpg(sn):
    ...
    now = datetime.now(SHANGAI) # current date and time
    folder_path = rawpath + now.strftime("%Y-%m-%d")
    print(folder_path)
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    jpg_name = now.strftime("%Y-%m-%d_%H-%M-%S") + ".jpg"
    sn = folder_path +'/'+ jpg_name
    fn = 'stream.h265'
    get_jpg_from_h265(fn,sn,0.3)
